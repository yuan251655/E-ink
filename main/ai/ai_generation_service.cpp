...
constexpr char kLastError[] = "error";
constexpr char kLastFinished[] = "finished";
constexpr char kLastProfileId[] = "profile_id";
constexpr char kLastProfileName[] = "profile_name";
constexpr char kLastPromptSummary[] = "prompt_summary";
constexpr char kLastPrompt[] = "prompt";
...
bool ReadNvsString(nvs_handle_t handle, const char* key, std::size_t maximum, std::string* output) {
    std::size_t length = 0;
    if (nvs_get_str(handle, key, nullptr, &length) != ESP_OK || length == 0 || length > maximum) return false;
    std::string value(length, '\0');
    if (nvs_get_str(handle, key, value.data(), &length) != ESP_OK) return false;
    value.resize(std::strlen(value.c_str())); *output = value; return true;
}
...
// JobService fingerprints are intentionally bounded. Never store the raw
// prompt there: a normal Chinese prompt can exceed the fixed 128-byte field,
// which would otherwise be misreported as an unrelated "busy" condition.
std::string RequestFingerprint(const char* scope, const std::string& payload) {
    unsigned char digest[32]{};
    mbedtls_sha256_context hash;
    mbedtls_sha256_init(&hash);
    const bool ok = mbedtls_sha256_starts(&hash, false) == 0 &&
        mbedtls_sha256_update(&hash, reinterpret_cast<const unsigned char*>(payload.data()), payload.size()) == 0 &&
...
struct DownloadContext { StorageService* storage=nullptr; std::uint64_t bytes=0; bool first=true; bool jpeg_soi=false; uint8_t tail_a=0; uint8_t tail_b=0; esp_err_t error=ESP_OK; mbedtls_sha256_context sha{}; };
struct DownloadResult { std::string code="ai_download_failed"; esp_err_t transport_error=ESP_FAIL; int http_status=0; int tls_error=0; esp_err_t storage_error=ESP_OK; std::uint64_t bytes=0; bool invalid_source=false; };
esp_err_t DownloadEvent(esp_http_client_event_t* event) { auto* context=static_cast<DownloadContext*>(event->user_data); if (event->event_id!=HTTP_EVENT_ON_DATA || event->data_len<=0) return ESP_OK; if (!context || !context->storage || context->bytes+static_cast<std::size_t>(event->data_len)>kMaxSourceBytes) return ESP_ERR_INVALID_SIZE; const auto* data=static_cast<const uint8_t*>(event->data); if(context->first) context->jpeg_soi=event->data_len>=2 && data[0]==0xff && data[1]==0xd8; for(int i=0;i<event->data_len;++i){ context->tail_a=context->tail_b; context->tail_b=data[i]; } if (mbedtls_sha256_update(&context->sha, data, event->data_len)!=0) return ESP_FAIL; context->error=context->storage->AppendStagedFile("source.jpg", event->data, event->data_len, context->first); context->first=false; if (context->error!=ESP_OK) return context->error; context->bytes += static_cast<std::size_t>(event->data_len); return ESP_OK; }
...
const char* DownloadFailureCode(const DownloadResult& result) {
    if (result.invalid_source) return "ai_source_invalid";
    if (result.storage_error == ESP_ERR_INVALID_SIZE) return "ai_source_too_large";
    if (result.storage_error != ESP_OK) return "ai_download_storage_failed";
    if (result.http_status >= 300 && result.http_status <= 399) return "ai_download_redirect_failed";
    if (result.http_status >= 400 && result.http_status <= 499) return "ai_download_http_4xx";
    if (result.http_status >= 500 && result.http_status <= 599) return "ai_download_http_5xx";
    if (result.tls_error != 0 || (result.transport_error >= ESP_ERR_ESP_TLS_BASE && result.transport_error < ESP_ERR_ESP_TLS_BASE + 0x100)) return "ai_download_tls_failed";
    if (result.transport_error == ESP_ERR_TIMEOUT || result.transport_error == ESP_ERR_ESP_TLS_CONNECTION_TIMEOUT || result.transport_error == ESP_ERR_ESP_TLS_SERVER_HANDSHAKE_TIMEOUT) return "ai_download_timeout";
    if (result.transport_error == ESP_ERR_HTTP_CONNECT || result.transport_error == ESP_ERR_ESP_TLS_CANNOT_RESOLVE_HOSTNAME || result.transport_error == ESP_ERR_ESP_TLS_FAILED_CONNECT_TO_HOST) return "ai_download_network_failed";
    return "ai_download_failed";
}
bool DownloadSource(StorageService* storage, const std::string& url, std::string* sha256, std::uint64_t* bytes, DownloadResult* result) {
    if (!storage || !sha256 || !bytes || !result) return false;
    DownloadContext context; context.storage=storage; mbedtls_sha256_init(&context.sha);
    if(mbedtls_sha256_starts(&context.sha,false)!=0) { mbedtls_sha256_free(&context.sha); result->storage_error=ESP_FAIL; return false; }
    esp_http_client_config_t config{}; config.url=url.c_str(); config.crt_bundle_attach=esp_crt_bundle_attach; config.event_handler=DownloadEvent; config.user_data=&context; config.timeout_ms=90000; config.buffer_size=4096; config.max_redirection_count=3;
    auto client=esp_http_client_init(&config); if(!client){mbedtls_sha256_free(&context.sha); result->storage_error=ESP_ERR_NO_MEM; result->code="ai_download_storage_failed"; return false;}
    esp_http_client_set_method(client,HTTP_METHOD_GET); result->transport_error=esp_http_client_perform(client); result->http_status=esp_http_client_get_status_code(client); int tls_flags=0; (void)esp_http_client_get_and_clear_last_tls_error(client,&result->tls_error,&tls_flags); esp_http_client_cleanup(client);
    unsigned char digest[32]{}; const bool hash_ok=mbedtls_sha256_finish(&context.sha,digest)==0; mbedtls_sha256_free(&context.sha); result->storage_error=context.error; result->bytes=context.bytes;
    result->invalid_source=!context.jpeg_soi || context.tail_a!=0xff || context.tail_b!=0xd9;
...
std::string AiGenerationService::SummarizePrompt(const char* prompt) {
    std::string result;
    if (prompt == nullptr) return result;
    for (const char* cursor = prompt; *cursor != '\0' && result.size() < 60; ++cursor) {
        const char ch = *cursor;
        result.push_back((ch == '\r' || ch == '\n' || ch == '\t') ? ' ' : ch);
    }
    if (std::strlen(prompt) > result.size()) result.append("...");
    return result;
}
void AiGenerationService::UpdateActivePhaseLocked(const char* phase) {
    xSemaphoreTake(reinterpret_cast<SemaphoreHandle_t>(mutex_), portMAX_DELAY);
    if (active_) { active_task_.state = "running"; active_task_.phase = phase ? phase : "running"; }
    xSemaphoreGive(reinterpret_cast<SemaphoreHandle_t>(mutex_));
}
AiGenerationService::ActiveTaskSnapshot AiGenerationService::GetActiveTaskSnapshot() const {
    ActiveTaskSnapshot output;
    xSemaphoreTake(reinterpret_cast<SemaphoreHandle_t>(mutex_), portMAX_DELAY);
    output = active_task_;
    xSemaphoreGive(reinterpret_cast<SemaphoreHandle_t>(mutex_));
    return output;
}
AiGenerationService::LastTaskSnapshot AiGenerationService::GetLastTaskSnapshot() const {
    LastTaskSnapshot output;
    xSemaphoreTake(reinterpret_cast<SemaphoreHandle_t>(mutex_), portMAX_DELAY);
    output = last_task_;
    xSemaphoreGive(reinterpret_cast<SemaphoreHandle_t>(mutex_));
    return output;
}
void AiGenerationService::LoadLastTask() {
    if (mutex_ == nullptr) return;
    nvs_handle_t handle;
    if (nvs_open(kDiagNamespace, NVS_READONLY, &handle) != ESP_OK) return;
    LastTaskSnapshot loaded;
    ...
    (void)ReadNvsString(handle, kLastProfileName, 65, &loaded.profile_name);
    (void)ReadNvsString(handle, kLastPromptSummary, 65, &loaded.prompt_summary);
    std::string preview_prompt;
    (void)ReadNvsString(handle, kLastPrompt, 769, &preview_prompt);
    nvs_close(handle);
    xSemaphoreTake(reinterpret_cast<SemaphoreHandle_t>(mutex_), portMAX_DELAY);
    last_task_ = std::move(loaded);
    // A preview is a user-visible pending decision, not disposable scratch
    // data. Restore the latest committed preview after reboot when its source
    // file is still present; the normal expiry window starts anew so a user
    // can finish reviewing it after a firmware update.
    if (last_task_.available && last_task_.kind == "generate_preview" &&
        last_task_.state == "success" && !preview_prompt.empty() &&
        storage_ != nullptr && storage_->PreviewFileExists(last_task_.job_id)) {
        preview_ = {last_task_.job_id, preview_prompt,
                    static_cast<std::uint64_t>(esp_timer_get_time() / 1000) + 10ULL * 60ULL * 1000ULL,
                    0, true};
    }
    xSemaphoreGive(reinterpret_cast<SemaphoreHandle_t>(mutex_));
}
void AiGenerationService::RecordTerminalTask(const Work& work, bool success, const std::string& error_code) {
    ...
    recorded.error_code = success ? "" : error_code;
    recorded.finished_at_ms = static_cast<std::uint64_t>(esp_timer_get_time() / 1000);
    recorded.prompt_summary = SummarizePrompt(work.prompt);
    const auto profile = config_->GetPublicConfig();
    recorded.profile_id = profile.profile_id;
    recorded.profile_name = profile.profile_name;
    nvs_handle_t handle;
    if (nvs_open(kDiagNamespace, NVS_READWRITE, &handle) == ESP_OK) {
        ...
        if (result == ESP_OK) result = nvs_set_str(handle, kLastError, recorded.error_code.c_str());
        if (result == ESP_OK) result = nvs_set_u64(handle, kLastFinished, recorded.finished_at_ms);
        if (result == ESP_OK) result = nvs_set_str(handle, kLastProfileId, recorded.profile_id.c_str());
        if (result == ESP_OK) result = nvs_set_str(handle, kLastProfileName, recorded.profile_name.c_str());
        if (result == ESP_OK) result = nvs_set_str(handle, kLastPromptSummary, recorded.prompt_summary.c_str());
        if (result == ESP_OK) result = nvs_set_str(handle, kLastPrompt, work.prompt);
        if (result == ESP_OK) (void)nvs_commit(handle);
        nvs_close(handle);
    }
    xSemaphoreTake(reinterpret_cast<SemaphoreHandle_t>(mutex_), portMAX_DELAY);
    last_task_ = std::move(recorded);
    xSemaphoreGive(reinterpret_cast<SemaphoreHandle_t>(mutex_));
}
esp_err_t AiGenerationService::Create(const RequestId& request_id, const std::string& prompt, bool /*ignored_display_flag*/, JobSnapshot* job, std::string* code) {
    ...
    const auto admission = jobs_->CreateOrFind(JobKind::kAiGeneration, request_id, RequestFingerprint("ai:", prompt), job);
    ...
    if (admission != JobRegistrationResult::kCreated) { xSemaphoreGive(reinterpret_cast<SemaphoreHandle_t>(mutex_)); *code=admission==JobRegistrationResult::kRequestIdConflict?"request_id_conflict":admission==JobRegistrationResult::kCapacityFull?"job_capacity_full":"invalid_request"; return ESP_ERR_INVALID_STATE; }
    ...
    active_=true; active_task_ = {true, job->job_id, "queued", "queued", "generate_preview", SummarizePrompt(work.prompt)};
    if (xQueueSend(reinterpret_cast<QueueHandle_t>(queue_), &work, 0) != pdTRUE) { active_=false; active_task_={}; (void)jobs_->Update(job->job_id, JobState::kFailed,"failed",0,"ai_job_busy"); xSemaphoreGive(reinterpret_cast<SemaphoreHandle_t>(mutex_)); *code="ai_job_busy"; return ESP_ERR_INVALID_STATE; }
...
esp_err_t AiGenerationService::ConfirmSave(const RequestId& request_id, const std::string& preview_job_id, JobSnapshot* job, std::string* code) {
    ...
    active_=true; active_task_ = {true, job->job_id, "queued", "queued", "confirm_save", SummarizePrompt(work.prompt)};
    if(xQueueSend(reinterpret_cast<QueueHandle_t>(queue_),&work,0)!=pdTRUE){active_=false;active_task_={};(void)jobs_->Update(job->job_id,JobState::kFailed,"failed",0,"ai_job_busy");xSemaphoreGive(reinterpret_cast<SemaphoreHandle_t>(mutex_));*code="ai_job_busy";return ESP_ERR_INVALID_STATE;} xSemaphoreGive(reinterpret_cast<SemaphoreHandle_t>(mutex_));*code="accepted";return ESP_OK;
}
    ...
    for (;;) {
        ...
        do {
            if (work.kind == WorkKind::kConfirmSave) {
                ...
                UpdateActivePhaseLocked("converting");
            ...
            if (work.kind == WorkKind::kGeneratePreview) {
                ...
                bool downloaded = DownloadSource(storage_, url, &source_hash, &source_bytes, &download_result);
                // The model request has already completed. A second GET of the
                // same signed source URL is free and can recover a transient
                // truncated transfer without creating another billable image.
                if (!downloaded && download_result.code == "ai_source_invalid") {
                    vTaskDelay(pdMS_TO_TICKS(800));
                    downloaded = DownloadSource(storage_, url, &source_hash, &source_bytes, &download_result);
                }
                if (!downloaded) { failure = download_result.code; break; }
            ...
            transaction_started = true;
            DownloadResult download_result;
            if (!DownloadSource(storage_, url, &source_hash, &source_bytes, &download_result)) { failure = download_result.code; break; }
